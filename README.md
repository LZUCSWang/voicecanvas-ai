...
当前版本已完成 React + TypeScript + Vite 工程脚手架、Vitest 基础测试、绘图 action 模型、SVG 画布渲染、基础状态界面、结构化场景模板和本地中文指令解析。页面可展示圆形、矩形、三角形、直线、箭头和文本对象，也可通过开发辅助输入框或预置按钮触发 flowchart、mind-map、comparison、architecture、poster 五类完整画面进行本地验证。

当前尚未实现真实语音识别、语音反馈、云端解析、后端能力、撤销历史或导出功能。开发辅助输入框只用于本地测试，不作为正式交互主入口。
...
- 原创功能部分：当前包含项目脚手架、绘图 action 模型、action executor、兼容自定义布局字段、SVG 画布渲染、结构化场景模板、本地中文指令解析、容错词归一化、多动作拆分、场景意图触发、基础状态面板、最近识别文本展示、动作历史和开发辅助预置 action；语音、后端、云端解析和导出将在后续小 PR 中实现。
- 复用代码来源：无。
